(function() {
    ...
    c.AmRadarChart = c.Class({
        ...
        drawChart: function() {
            ...
            this.realRadius = c.toCoordinate(this.radius, Math.min(a - f - l, b - d - k), e);
            ...
            if (c.ifArray(a)) {
                if (0 < this.realWidth && 0 < this.realHeight) {
                    ...
                    d = this.graphs;
                    for (b = 0; b < d.length; b++) d[b].zoom(0, a); (a = this.legend) && a.invalidateSize()
                }
    ...
    c.RadAxis = c.Class({
        construct: function(a) {
            ...
            if (b) {
                ...
                A = u.length,
                w, n = this.axisWidth;
                "middle" == a.pointPosition && "circles" != a.gridType && (a.rMultiplier = Math.cos(180 / A * Math.PI / 180), n *= a.rMultiplier);
                for (w = 0; w < A; w++) {
                    var t = 180 - 360 / A * w,
                    q = t;
                    ...
                    if (g) {
                        ...
                        if (180 == q || 0 === q) B = "middle",
                        v -= 5;
                        0 > q && (B = "end", v -= 10);
                        180 == q && (t -= 5);
                        0 === q && (t += 5);
                        q = c.text(b.container, u[w].category, r, h, m, B);
                        q.translate(v + 5, t);
                        this.set.push(q);
    ...
    c.RadItem = c.Class({
        construct: function(a, b, d, f, k, l, e, p) {
            ...
            H && (K = "start", F = 1);
            var y;
            G && (y = c.text(r, d, m, g, h, K, p), y.translate(D + (w + 3 + L) * F, b), k.push(y), c.setCN(f, y, a.bcn + "label"), e && c.setCN(f, y, "guide"), c.setCN(f, y, C, !0), this.label = y, z = c.line(r, [D, D + w * F], [b, b], u, A, t), k.push(z), c.setCN(f, z, a.bcn + "tick"), e && c.setCN(f, z, "guide"), c.setCN(f, z, C, !0));
            b = Math.round(a.y - b);
            p = [];
            g = [];
            if (0 < n) {
                if ("polygons" == I) {
                    x = a.data.length;
                    for (h = 0; h < x; h++) m = 180 - 360 / x * h,
                    p.push(b * Math.sin(m / 180 * Math.PI)),
                    g.push(b * Math.cos(m / 180 * Math.PI));
                    p.push(p[0]);
                    g.push(g[0]);
            ...
            if (1 == l && 0 < E && !e && "" !== d) {
                e = a.previousCoord;
                if ("polygons" == I) {
                    for (h = x; 0 <= h; h--) m = 180 - 360 / x * h,
                    p.push(e * Math.sin(m / 180 * Math.PI)),
                    g.push(e * Math.cos(m / 180 * Math.PI));
    ...
    c.RadarFill = c.Class({
        construct: function(a, b, d, f) {
            ...
            if ("polygons" == a.gridType) {
                ...
                r = a.data.length,
                u;
                for (u = 0; u < r; u++) g = 180 - 360 / r * u,
                h.push(k * Math.sin(g / 180 * Math.PI)),
                m.push(k * Math.cos(g / 180 * Math.PI));
                h.push(h[0]);
                m.push(m[0]);
                for (u = r; 0 <= u; u--) g = 180 - 360 / r * u,
                h.push(b * Math.sin(g / 180 * Math.PI)),
                m.push(b * Math.cos(g / 180 * Math.PI));
