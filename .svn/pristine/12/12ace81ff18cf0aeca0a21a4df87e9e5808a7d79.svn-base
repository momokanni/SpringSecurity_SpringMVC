...
function _getAttr(el, key) {
	...
	if (!_GET_SET_ATTRIBUTE && el.nodeName.toLowerCase() != 'script') {
		...
		try{
			//这里try是因为在ie6下editor这里会有一个object报错 ( div.innerHTML == "<object>...</object>" ) div.firstChild==OBJECT, 得不到属性
...
_extend(KUploadButton, {
	init : function(options) {
		var self = this,
			...
			url = options.url || '/admin/richUpload.action',
			title = button.val(),
...
KEditor.prototype = {
	...
	loadPlugin : function(name, fn) {
		...
		self.isLoading = true;
		_loadScript(self.pluginsPath + name + '/' + name + '.js?ver=' + encodeURIComponent(K.DEBUG ? _TIME : _VERSION) + "&t=" + time_stamp, function() {
			self.isLoading = false;
			if (_plugins[name]) {
	...
	create : function() {
		...
		var toolbar = self.toolbar = _toolbar({
			src : toolbarDiv,
			display : self.options.toolbarDisplay,
			html : htmlList.join(''),
		...
		var edit = self.edit = _edit({
			height : editHeight > 0 && _removeUnit(height) > self.minHeight ? editHeight : editHeight,
			src : editDiv,
		...
		if (self.options.statusbarDisplay) {
...
options.themesPath = options.themesPath.replace('scripts', 'styles');
		_loadStyle(options.themesPath + 'default/default.css?' + time_stamp);
    
		_loadStyle(options.themesPath + themeType + '/' + themeType + '.css?' + time_stamp);
	}
	...
	var knode = K(expr);
	//console.log("expression is:",expr," & id is:",knode[0].id);
	if (!knode) {
		return;
	}
	if (knode.length > 1) {
...
_plugin('core', function(K) {
	...
	self.clickToolbar('fullscreen', function() {
        ...
        if (typeof isFullscreen === 'undefined') {
          ...
          delete isFullscreen;
          if (_IE) {
            $('.ke-container-default').css('width', '98%');
          }
        }
	});
	...
	self.afterCreate(function() {
		...
		function movePastedData() {
            ...
            K('[style]', div).each(function() {
                if (K(this).css('white-space') == 'nowrap') {
                    K(this).css('white-space', '');
                }
            });
