...
QNR.TableList=(function(){
    ...
    q.TableList = {
        ...
        query : function(queryURL){
            ...
            $.getJSON(url,function(data){
                if(!!data && !!data.ret){
                    ...
                    self.drawTemplete(data.data,self.setting.targetContent);
                }else{
                	// 无数据情况下的显示信息
                    var containter = '<div style="width:100%;font-weight:bold;font-size:14px;text-align:center;margin-top:40%;"><img style="width: 24%;margin: 0 auto;" src="http://img.91caishen.com/source/images/contpla/app/qs.png"><p>'+data.errmsg +'</p></div>';
                    var node = null;
                    if(self.setting.targetContent == "followUL"){
                        node = mui('#focusNone')[0];
                        node.className = "clearfix widbig nodata";
                    } else if(self.setting.targetContent == "ulview"){
                        node = mui('.mui-pull-bottom-tips')[0];
                        mui('#' + self.setting.targetContent)[0].innerHTML = containter;
                        $("#lable").addClass("dn");
                        mui('#pullrefresh').pullRefresh().disablePullupToRefresh();
                        //node.setAttribute("style","display:none;");
                    } else {
                    	$('div [target='+ self.setting.targetContent + ']').empty();
                        $('div [target='+ self.setting.targetContent + ']').html(containter);
                    	// 隐藏 上拉获取数据div
                        node = $('div [target='+ self.setting.targetContent + ']')[0].parentNode.lastChild;
                        node.setAttribute("style","display:none;");
                    }
                    mui('.mui-scroll-wrapper').scroll({})[0];
                }
            	...
            	// 无数据情况下的显示信息
                var containter = '<div style="width:100%;font-weight:bold;font-size:14px;text-align:center;margin-top:50%;background-color: #ffffff;"><p>请求超时</p></div>';
                var node = null;
                if(self.setting.targetContent == "followUL"){
                    node = mui('.mui-pull-bottom-tips')[0];
                    mui('.mui-scroll .mui-table-view')[0].innerHTML = containter;
                    node.setAttribute("style","display:none;");
                } else if(self.setting.targetContent == "ulview"){
                    node = mui('.mui-pull-bottom-tips')[0];
                    mui('#' + self.setting.targetContent)[0].innerHTML = containter;
         ...
         * 数据部分渲染 
         */
        drawTemplete : function(d,target){
        	var data = [];
        	var content = [];
            ...
            if(!$("#"+q.TableList.setting.tempId).length){
               ...
               for(var c  in d){
                ...
                if(target == "followUL"){
                ...
                } else if(target==="ulview"){
                    $("#lable").addClass("dn");
                    tmpl.appendTo(".mui-table-view");
                	...
                	var node=$('div [target='+ target + ']')[0];
                    node.innerHTML = "";
                    tmpl.appendTo('div [target='+ target + ']');
                }
