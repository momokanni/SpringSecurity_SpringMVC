function DragAudioUpload(id,options) {
    this.me = $(id);
    var defaultOpt = {
        boxWidth:'200px',
        boxHeight:'auto'
    }
    this.preview = $('<div id="preview"><img src="/images/manager/audio.png" class="audio-responsive" alt="" title="" style="width:60px;height:60px;padding-left:35%;" /><span id="audioFileName" style="display:none;"></span></div>');
...
DragAudioUpload.prototype = {
    ...
    onDrop:function (e) {
        ...
        //检测文件是不是图片
        if(fileList[0].type.indexOf('mp3') === -1){
            alert("您拖的不是音频！");
            return false;
        }
        ...
        self.me.find("span").html(filename);
        if(this.callback){
    ...
    onChangeUploadFile:function () {
        ...
        this.me.find("span").html(filename);
        if(this.callback){
            this.callback(files);
        }
    },
