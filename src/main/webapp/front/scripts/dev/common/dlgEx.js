...
Dlg.prototype = {
    ...
    _init: function() {
    		if (this.iKj) {
    			...
    			h.push("<div id='modal-full-width' class='modal fade' data-backdrop='static' tabindex='-1' >");
				h.push("	<div class='modal-header'>");
				h.push("		<button type='button' class='close' data-dismiss='modal' aria-hidden='true'></button>");
    			...
    			top.$("#modelDv").html(h.join(""));
    			
    			top.$("#modal-full-width").attr("data-width", this.width);
    		} else {
    			 //dlg
    ...
    setHeader: function(s) {
    	if (this.iKj) {
    		top.$("#modal-full-width").find(".modal-title").html(s);
    	} else {
    		this.header.html(s);
    	}
    ...
    setBody: function(s){
    	if (this.iKj) {
    		top.$("#modal-full-width").find(".modal-body").html(s);
    	} else {
    		this.body.html(s);
    	}
    ...
    setBtns: function(btns){
    	if (this.iKj) {
    		top.$("#modal-full-width").find(".modal-footer").empty();
            var self = this;
    		$(btns).each(function(i, btn) {
    			var btnEl;
    			if (btn.close != null) {
    				btnEl = $("<button type='button'  class='btn btn-outline dark'></button>").html(btn.text);
    			} else {
    				btnEl = $("<button type='button' class='btn green'></button>").html(btn.text);
    			}
                ...
                top.$("#modal-full-width").find(".modal-footer").append(btnEl);
            });
    	} else {
    ...
    show: function() {
    	...
    	if (this.iKj) {
    		var dw = top.$("#modal-full-width").attr("data-width");
    		
    		top.$("#modal-full-width").modal();
    		top.$("#modal-full-width").css("width",dw);
    	} else {
...
//    	        }
    ...
    close: function(f) {// f是否执行closeFunc
    	if (this.iKj) {
    		top.$("#modal-full-width").modal("hide");
    	} else {
