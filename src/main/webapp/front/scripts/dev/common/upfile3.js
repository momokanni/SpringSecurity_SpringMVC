...
var _addImage = function(tar , imageData) {
	...
	var img = "";
	if (imageData.wx) {
		var localId = imageData.localId;
		var serverId = imageData.serverId;
		if (localId == null || localId == "") {
			img = "<a style='display:none' wx='d' gallery='' class='fl pr' fl='t' href='javascript:void(0)' ><img src='' class='imgaut' v='" + serverId + "' /><div style='position:absolute; top:0; left:0; width:100%; height:100%; z-index:1;-webkit-touch-callout:none; -webkit-user-select:none; -khtml-user-select:none; -moz-user-select:none;  -ms-user-select:none;  user-select:none; '></div></a>";
		} else {
			img = "<a wx='t' gallery='"+ localId + "' class='fl pr' fl='t' href='javascript:void(0)' ><img src='" + localId + "' class='imgaut' v='" + serverId + "' /><div style='position:absolute; top:0; left:0; width:100%; height:100%; z-index:1;-webkit-touch-callout:none; -webkit-user-select:none; -khtml-user-select:none; -moz-user-select:none;  -ms-user-select:none;  user-select:none; '></div></a>";
		}
	} else {
		img = "<a wx='f' gallery='"+ fileServer + imageData.imgOri + "' class='fl pr' fl='t' href='javascript:void(0)' ><img src='" + fileServer + imageData.resize + "' class='imgaut' v='" + imageData.imgKey + "' /><div style='position:absolute; top:0; left:0; width:100%; height:100%; z-index:1;-webkit-touch-callout:none; -webkit-user-select:none; -khtml-user-select:none; -moz-user-select:none;  -ms-user-select:none;  user-select:none; '></div></a>";
...
var wxDownload = function(serverId, tar) {
//	mobileNotice(serverId);
	wx.downloadImage({
		serverId: serverId, 
	    isShowProgressTips: 0, // 默认为1，显示进度提示
	    success: function (res) {
	    	var localId = res.localId;
//	    	mobileNotice(localId);
	    	var imgData = {
	        		wx : true,
	        		localId : localId,
	        		serverId : serverId
...
//	        mobileNotice("1:" + serverId);
	        wxDownload(serverId, tar);
	        //mobileNotice(serverId);
	        /*
	        $.ajax({  
		        url: "/wx/wxUpload",  
	...
	if (__twx) {
		wx.chooseImage({
		    ...
		    success: function (res) {
		        ...
		        //wxLocalResource://50114659201332
...
var _tryDown = function() {
	
	var cl = setInterval(function() {
//		mobileNotice(_wdy);
		if (_wdy) {
			clearInterval(cl);
			var objs = $("a[gallery][wx=d]");
//			mobileNotice(wxds.length);
			if (objs.length > 0) {
				var wxds = [];
...
var _downWxImg = function(wxds) {
	var obj = wxds.pop();
	var serverId = $(obj).find("img").attr("v");
//	mobileNotice(serverId);
	var self = obj;
	wx.downloadImage({
		serverId: serverId, 
	    ...
	    success: function (res) {
	    	var localId = res.localId;
	    	$(self).attr("wx", "t");
	    	$(self).attr("gallery", localId);
	    	$(self).find("img").attr("src", localId);
	    	$(self).css("display","");
