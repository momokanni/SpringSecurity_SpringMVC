...
public class StringUtil {
	...
	public static String setValueWhenNull(String value){
		if(isEmpty(value)){
			value = "";
		}
		return value;
...
//		}
	 ...
	 */
	public static String escapeUrlXss(String content){
		if(StringUtil.isEmpty(content))
			return content;
		try {
			return ESAPI.encoder().encodeForURL(content);
	...
	public static String escapeHtmlXss(String content){
		if(StringUtil.isEmpty(content))
			return content;
	...
	* @开发时间： 2014-9-12 下午4:40:50
	...
	public static String fenGe(double money) {
	 ...
	 * 
	* @描述：把html转化成字符串
	* @开发时间： 2014-4-18 下午4:31:05
	 */
	public static String getDescsStr(String string) {
		NekoHtmlParser parser = new NekoHtmlParser();
		parser.load("<div id='abc'>" + string + "</div>", "utf-8");
		Node n = parser.selectSingleNode("//DIV[@id='abc']");
		List<String> descs = getDescs(n);
		StringBuilder sb = new StringBuilder();
	...
	public static void main(String[] str) throws Exception {
		BigDecimal pi = new BigDecimal("3.10");
		System.out.println(FormatUtil.formatDoubleNoDotZero(pi));
   ...
   public static String menuTOJsonString(Map<Integer,List<SysPermit>> menuMap){
	   if(menuMap == null || menuMap.isEmpty())
		   return "{}";
	   Map<String,Map<String,Integer>> smenuMap = new HashMap<String,Map<String,Integer>>();
	   for(Map.Entry<Integer,List<SysPermit>> entry : menuMap.entrySet()){
		   List<SysPermit> splist = entry.getValue();
		   for(SysPermit sp : splist){
			   if(isEmpty(sp.getShortCode()))
				   continue;
			   Map<String,Integer> tmap = new HashMap<String,Integer>();
			   tmap.put("m", entry.getKey());
			   tmap.put("sm", sp.getId());
			   smenuMap.put(sp.getShortCode(), tmap);
		   }
	   }
	   
	   ObjectMapper om = new ObjectMapper();
	   try{
		   return om.writeValueAsString(smenuMap);
	   }catch(Exception e){
		   return "{}";
	   }
   }
...
//		if (src.matches("^[^\\d]+\\d{11}$")) {
//			Matcher m = Pattern.compile("([^\\d]+)(\\d{2})(\\d{6})(\\d{3})").matcher(src);
//			if (m.find()) {
//				return m.group(1) + m.group(2) + "***" + m.group(4);
//			}
//		}
		//return maskString(src);
    ...
    public static Map parseJsonMap(String src) {
    	ObjectMapper om = new ObjectMapper();
    	Map ret = null;
		try {
			Map m = om.readValue(src, Map.class);
			ret = m;
		} catch (Exception e ) {
		}
    ...
    public static String parseJsonString(Map m) {
    	ObjectMapper om = new ObjectMapper();
		String ret2 = null;
		try {
			ret2 = om.writeValueAsString(m);
		} catch (Exception e) {
			e.printStackTrace();
		} 
		return ret2;
    ...
    public static String genApiKey(int deptId) {
    	String idEnc = IDEncryptor.getInstance().encryptWithoutException(deptId);
    	String desEnc = "";
		try {
			desEnc = Des.encrypt(idEnc);
		} catch (Exception e) {
			e.printStackTrace();
		}
    ...
    public static int parseApiKey(String key) {
    	String idEnc = "";
    	try {
			idEnc = Des.decrypt(key);
		} catch (Exception e) {
			e.printStackTrace();
		}
    	
    	int id = IDEncryptor.getInstance().decryptWithoutException(idEnc);
    	
    	return id;
    }
    
    public static String handleEmpty(String src) {
    	if (src == null) {
    		return "";
    	}
    	return src;
	...
	public static String outputToJudge(Object obj) {
